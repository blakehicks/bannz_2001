...
const utils = {
  ...
  getBanners: function () {
    ...
    if (banners.length == 1 && banners[0] === '_banner_template') {
      console.log(
        "\n//There's nothing to watch! Run " +
          chalk.magenta('bannz setup') +
          " if you haven't yet.\n"
      );
      process.exit();
    }
...
async function watchDir() {
  ...
  let titleRegExp = new RegExp('<head>[\\s\\S]*?<title>(.*?)</title>', 'i');
  let indexContent = currentDirectory.read('index.html').toString();
  indexContent = indexContent.replace(
    adSizeRegExp,
    'content="width=' + size.width + ', height=' + size.height + '"'
  );
  indexContent = indexContent.replace(titleRegExp, (match, p1) =>
    match.replace(p1, project.title + ' | ' + size.width + 'x' + size.height)
  );
...
async function watch() {
  ...
  //ASSETS
  chokidar.watch(watchFolder + paths.img).on('change', (event, path) => {
    assets();
    console.log(
      '\n' +
        chalk.magenta('//Image Assets') +
...
exports.describe =
  'Choose creative to view in browser and watch for changes.\nUsing ' +
  chalk.magenta('watch --folder <creative name here>') +
  ' will skip the selection step.';
